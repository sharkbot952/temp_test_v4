...
# -----------------------------
    ...
    with st.sidebar.expander("任意データの追加（csv）", expanded=False):
        uploaded_generic = st.file_uploader("CSVを追加", type=["csv"], accept_multiple_files=True)
    ...
    with st.sidebar.expander("幼生データの追加（csv）", expanded=True):
        uploaded_size_files = st.file_uploader("CSVを追加", type=["csv"], accept_multiple_files=True)
        #size_series_filter  = st.text_input("seriesフィルタ（空=全て、例：w_temp）", value="")
        #bar_max_fraction    = st.slider("横棒の最大長", 0.1, 1.0, 1.0, step=0.05)
