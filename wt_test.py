...
# =========================================
try:
    view_mode = st.segmented_control("表示モード", options=["予測カレンダー", "水温グラフ"], default="予測カレンダー")
except Exception:
...
def safe_merge_asof_by_depth(
    ...
    out_list = []
    common_depths = sorted(set(left["depth_m"].dropna().unique()).intersection(set(right["depth_m"].dropna().unique())))
    for d in common_depths:
        ...
        merged = pd.merge_asof(l, r, on="datetime", by="depth_m", tolerance=tolerance, direction="nearest", suffixes=suffixes)
...
def summarize_daily_layer_flow(
    layer_name: str, layer_depths: List[int], df_day: pd.DataFrame,
    use_short_labels: bool = True, merge_same_segments: bool = False
) -> Optional[str]:
    ...
    if merge_same_segments:
        ...
        for (d, v), lbls in bucket.items():
            ...
            inner = "・".join([x for x in [d, v] if x]); segments.append(f"{'・'.join(lbls_sorted)}（{inner}）")
    else:
        rows_sorted = sorted(rows, key=lambda r: order.get(r[0], 99))
        for lbl, d, v in rows_sorted:
            inner = "・".join([x for x in [d, v] if x]); segments.append(f"{lbl}（{inner}）")
...
def render_cell_html(
    temp: Optional[float], speed_mps: Optional[float], dir_deg: Optional[float],
    use_correction: bool, corr_temp_raw: Optional[float], corr_temp_clip: Optional[float],
    bg_basis: str, hide_outlier_cells: bool, is_invalid: bool,
) -> str:
    if is_invalid:
        if hide_outlier_cells: return "<td></td>"
        else:
...
def build_weekly_table_html(
    df_period: pd.DataFrame, day_list: List[pd.Timestamp], depths_for_table: List[int],
    use_correction: bool, reg_depthwise: Optional[Dict[int, Tuple[float, float]]],
    bg_basis: str, hide_outlier_cells: bool, outlier_th: float,
) -> str:
    PHYS_MIN, PHYS_MAX = -1.5, 35.0
    ...
    for depth in depths_for_table:
        ...
        for day in day_list:
            ...
            if not g.empty:
                ...
                html += render_cell_html(temp, speed_val, dir_val, use_correction, corr_raw, corr_clip, bg_basis, hide_outlier_cells, is_invalid)
            else:
                html += "<td>-</td>"
        html += "</tr>\n"
...
def build_daily_table_html(
    df_day: pd.DataFrame, hours_list: List[pd.Timestamp], times_hr: List[str], depths_for_table: List[int],
    use_correction: bool, reg_depthwise: Optional[Dict[int, Tuple[float, float]]],
    bg_basis: str, hide_outlier_cells: bool, outlier_th: float,
) -> str:
    PHYS_MIN, PHYS_MAX = -1.5, 35.0
...
if view_mode == "予測カレンダー":
    ...
    if mode_view == "週間予測（表示値は昼頃）":
        ...
        if use_correction:
            ...
            with st.spinner(f"回帰補正パラメータ算出中（{selected_file}, 終端={selected_day:%Y-%m-%d}, 遡り{train_days}日）..."):
                reg_depthwise, n_match_reg = compute_depthwise_regression(
                    base_dir, selected_file, tolerance_min, start_dt=train_start_dt, end_dt=train_end_dt, min_pairs=5
                )
            if reg_depthwise is None:
                st.warning("回帰係数の算出に失敗（一致データ不足など）。補正なしで表示します。")
                use_correction = False

        st.markdown(f"**{start_day:%m/%d}～{end_day:%m/%d}までの推移（期間内の全時間）**")
        ...
        if use_correction:
            ...
            with st.spinner(f"回帰補正パラメータ算出中（{selected_file}, 終端={selected_day:%Y-%m-%d}, 遡り{train_days}日）..."):
                reg_depthwise_sel, n_match_reg_sel = compute_depthwise_regression(
                    base_dir, selected_file, tolerance_min, start_dt=sel_train_start_dt, end_dt=sel_train_end_dt, min_pairs=5
                )
...
# -----------------------------
    ...
    use_correction = st.sidebar.checkbox("実測ベース補正", value=False)
    tolerance_min  = st.sidebar.slider("時刻差の許容範囲（分）", 5, 120, 35, step=5)
    show_obs_points = st.sidebar.checkbox("実測（OBS）を重ねて表示", value=True)  # ← 点描画のオン/オフ専用
    ...
    if show_obs_points and (not df_period.empty) and (not df_obs_period.empty):
        ...
        merged_for_points = safe_merge_asof_by_depth(left=left, right=right, tolerance=tol, right_value_cols=["obs_temp"], suffixes=("","")).dropna(subset=["obs_temp"])

    # 表示水深の候補
    depth_options = sorted([int(d) for d in (df_period["depth_m"].dropna().unique() if not df_period.empty else [])])

    # 補正ON時は、期間内に実測がある水深のみに制限（点表示ON/OFFに関係なく）
    if use_correction:
        ...
        if not obs_depths:
            ...
            depth_options = [d for d in depth_options if d in obs_depths]  # 実測あり水深のみ
    ...
    if use_correction and not selected_depths:
        st.info("補正ONのため、期間内に実測が存在する水深のみ表示します。該当水深がありませんでした。")
        # ただし、ここでは停止しない（ユーザーが期間やファイルを変える前提）
    ...
    if show_obs_points and not merged_for_points.empty and selected_depths:
        for d in selected_depths:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=6, color=obs_color, symbol=symbol, line=dict(color="white", width=1.3)),
                opacity=0.92,
                hovertemplate="%{x}<br>水深: " + f"{d}m" + "<br>実測水温: %{y:.2f} °C<extra></extra>"
            ))

    show_legend = st.checkbox("凡例を表示", value=True)
    legend_cfg = dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0, font=dict(size=12), itemsizing="constant")
    fig.update_layout(
