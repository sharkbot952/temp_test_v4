...
def get_arrow_svg(direction_deg, speed_mps):
    ...
    # 注意：CSS中の{}はf-string衝突を避けるために二重の{{}}にする
    return f"""
...
def load_dr_single_file(base_dir: str, filename: str) -> pd.DataFrame:
    ...
    if ("U" in df.columns) and ("V" in df.columns):
        ...
        # arctan2(y, x) の引数順は (U, V) の元コードを踏襲
        ...
        for depth in depths_for_table:
            ...
            for idx in range(len(times)):
                ...
                if not row.empty:
                    ...
                    pred_html = (
                        f"<span class='pred-small'>{pred_label}</span>"
                        if use_correction else f"<span>{pred_label}</span>"
                    )
                    ...
                    cell_content = (
                        ...
                        f"{pred_html}"
                        f"{(f\"<span style='font-size:12px;color:#444;'>{speed_kt_label}</span>\" if speed_kt_label else '')}"
                        f"{(f\"<span style='display:block;line-height:1;margin:0;padding:0;'>{arrow_svg}</span>\" if arrow_svg else '')}"
                        f"{(f\"<span style='color:#D32F2F;font-weight:700;font-size:14px;margin:0;padding:0;\">{corr_value_html}</span>\" if (use_correction and corr_value_html) else '')}"
                        "</div>"
                    )
