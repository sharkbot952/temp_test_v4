...
# =========================================
# 水温グラフ（B案：補正ONで予測線を薄く細く、補正線を水深別色で強調）
# =========================================
    ...
    # --- 任意CSV・サイズCSV エクスパンダ（ラジオ直下にスライダーが来るよう並べ替え） ---
    ...
    def add_size_horizontal_bars_binned20(
        ...
        end_day,
        series_filter: str = "",
        bar_max_fraction: float = 1.0,
        vmax_full_day: float = 1.0,
        max_span_days: int = 2,
        row: int = 2,
        col: int = 1
    ):
        ...
        vmax = max(float(vmax_full_day), 1e-9)
        ...
        agg["x_start"] = pd.to_datetime(agg["date_day"])
        ...
        for _, r in agg.iterrows():
            ...
            fig.add_shape(type="rect", x0=x0, x1=x1, y0=y0, y1=y1,
                          ...
                          line=dict(color=border_black, width=0.8), fillcolor=fillcolor)
            vtxt = f"{float(r['value_sum']):.3f}"
            fig.add_trace(go.Scatter(
                x=[x1], y=[(y0 + y1)/2.0], mode="markers",
    ...
    # B案のパラメータ
    PRED_LINE_WIDTH_ON  = 1.0
    PRED_OPACITY_ON     = 0.35
    PRED_LINE_WIDTH_OFF = 2.0
    PRED_OPACITY_OFF    = 1.0
    CORR_LINE_WIDTH     = 3.0
    CORR_OPACITY        = 1.0
    CORR_DASH           = "solid"   # 好みで "dot" / "dash" に変更可

    def emphasize_color(hex_color: str) -> str:
        """予測の基本色を少し明るくした同系色にして補正線を強調"""
        try:
            rr = int(hex_color[1:3], 16); gg = int(hex_color[3:5], 16); bb = int(hex_color[5:7], 16)
            lift = 25
            rr = min(255, rr + lift); gg = min(255, gg + lift); bb = min(255, bb + lift)
    ...
    if not df_period.empty and selected_depths:
        for d in selected_depths:
            ...
            x = g["datetime"]
            y_raw = g["pred_temp"].astype(float)

            base_col  = color_map[int(d)]
            corr_col  = emphasize_color(base_col)
            lg        = f"depth{int(d)}"  # 凡例グループ（深さごと）

            pred_line_width = PRED_LINE_WIDTH_ON  if use_correction else PRED_LINE_WIDTH_OFF
            pred_opacity    = PRED_OPACITY_ON     if use_correction else PRED_OPACITY_OFF

            # 予測線（補正ONなら薄く細く）
            fig.add_trace(go.Scatter(
                x=x, y=y_raw, mode="lines",
                name=f"{d}m 予測", legendgroup=lg, showlegend=True,
                line=dict(color=base_col, width=pred_line_width),
                opacity=float(pred_opacity),
            ...
            if use_correction and (reg_depthwise is not None) and (int(d) in reg_depthwise):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f"{d}m 回帰補正", legendgroup=lg, showlegend=True,
                    line=dict(color=corr_col, width=CORR_LINE_WIDTH, dash=CORR_DASH),
                    opacity=float(CORR_OPACITY),
            ...
            for col in cols:
                ...
                if generic_plot_type == "日別棒(平均)":
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        line=dict(color=color, width=2), opacity=0.65,
                        hovertemplate="%{x}<br>" + f"{col}: " + "%{y:.3f}<extra></extra>"
                    ), row=1, col=1, secondary_y=True)
    ...
    # --- レイアウト ---
    show_legend = st.checkbox("凡例を表示", value=True, key=f"{view_mode}_show_legend_temp")
