        ...
        for day in day_list:
            ...
            if not g.empty:
                ...
                html += render_cell_html(
                    temp=temp, speed_mps=speed_val, dir_deg=dir_val, use_correction=use_correction,
                    corr_temp_raw=corr_raw, corr_temp_clip=corr_clip,
...
# -----------------------------
    ...
    use_correction = st.sidebar.checkbox("実測ベース補正", value=False)
    tolerance_min  = st.sidebar.slider("時刻差の許容範囲（分）", 5, 120, 35, step=5)
    show_obs_points = st.sidebar.checkbox("実測（OBS）を重ねて表示", value=True)  # ← 依存関係のため取得位置を前倒し

    # ★ 依存関係：補正ONならOBS表示も必須
    if use_correction and not show_obs_points:
        st.info("補正ONのため、期間内に実測が存在する水深のみ表示します。該当水深がありませんでした。")
        st.caption("補正を使用するには「実測（OBS）を重ねて表示」をONにしてください。")
        st.stop()
    ...
    # 表示水深（補正ON時は、OBS表示ONのときだけ「実測あり水深」に限定）
    ...
    if use_correction and show_obs_points:
        depth_options = [d for d in depth_options if d in obs_depths]  # 実測がある水深のみ（OBS表示ON時）
    ...
    if not df_period.empty and selected_depths:
        for d in selected_depths:
            ...
            if use_correction:
                # 係数があり、OBS表示ONで実測が存在する水深のみ補正線を描画
                if (reg_depthwise is not None) and (int(d) in reg_depthwise) and (show_obs_points and (int(d) in obs_depths)):
