# wt_test.py
# ------------------------------------------------------------
# 予測カレンダー（スマホ対応・nonlocal撤廃版）
# ・layout="wide" で横幅確保
# ・テーブルを .table-wrap でラップして横スクロール対応
# ・sticky ヘッダー＆先頭列を CSS で固定
# ・スマホ（768px以下）では速度・矢印を非表示にする
# ・余分な閉じタグ削除
# ・回帰係数は st.session_state に保持（nonlocal 不使用）
# ------------------------------------------------------------
...
# セッション状態（回帰係数など）
# =========================================
if "reg_depthwise" not in st.session_state:
    st.session_state.reg_depthwise = None
if "n_match_reg" not in st.session_state:
    st.session_state.n_match_reg = None
# =========================================
# サイドバー（表示モード）
# =========================================
...
if mode == "予測カレンダー":
    ...
    if use_correction:
        ...
        if reg_depthwise is None:
            ...
            st.session_state.reg_depthwise = reg_depthwise
            st.session_state.n_match_reg = n_match_reg
    ...
    if use_correction and (st.session_state.reg_depthwise is not None) and hide_uncorrected_depths:
        depths_for_table = [d for d in depths_for_table if int(d) in st.session_state.reg_depthwise]
...
"""
    ...
    def build_week_table() -> str:
        reg_depth = st.session_state.reg_depthwise  # 参照のみ
        # ヘッダー（日付のみ）
        ...
        for depth in depths_for_table:
            ...
            for day in day_list:
                ...
                if not row.empty:
                    ...
                    if use_correction and (reg_depth is not None):
                        if pd.notna(temp) and (int(depth) in reg_depth):
                            alpha, beta = reg_depth[int(depth)]
    ...
    def build_day_table() -> str:
        ...
        if use_correction:
            ...
            if reg_depthwise_sel is None:
                ...
                # セッションに保存（以降の表示処理で参照）
                st.session_state.reg_depthwise = reg_depthwise_sel
                st.session_state.n_match_reg = n_match_reg_sel
                with st.expander(f"補正の学習情報（終端={selected_day:%Y-%m-%d}・遡り{train_days}日）", expanded=False):
                    ...
                    st.write("一致ペア数:", {int(k): int(v) for k, v in (st.session_state.n_match_reg or {}).items()})
                    st.write(
                        "学習済み水深（α, β）:",
                        {
                            int(k): (round(v[0], 4), round(v[1], 4))
                            for k, v in (st.session_state.reg_depthwise or {}).items()
                        }
                    )

        reg_depth = st.session_state.reg_depthwise  # 参照
