            ...
            if not g.empty:
                ...
                if use_correction and (corr_raw is not None):
                    if (corr_raw <= PHYS_MIN) or (corr_raw >= PHYS_MAX):
                        is_invalid = True
                if not is_invalid:
                    if (corr_clip is not None) and ((corr_clip <= TEMP_MIN) or (corr_clip >= TEMP_MAX)):
                        is_invalid = True
...
# -----------------------------
    ...
    if use_correction:
        depth_options = [d for d in depth_options if d in obs_depths]  # 実測がある水深のみ
    ...
    if not df_period.empty and selected_depths:
        for d in selected_depths:
            ...
            if use_correction:
                # 実測があって係数がある水深のみ補正線を描画
                if (reg_depthwise is not None) and (int(d) in reg_depthwise) and (int(d) in obs_depths):
