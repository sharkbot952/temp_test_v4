...
def normalize_generic_uploaded_csv(file) -> pd.DataFrame:
    ...
    # 数値列化（Date/datetime/Depth/depth_m 以外）
    for c in df.columns:
...
# -----------------------------
    ...
    def add_size_horizontal_bars_binned20(
        ...
        end_day,
        series_filter: str = "",
        bar_max_fraction: float = 1.0,  # 最大長（1日比）
        vmax_full_day: float = 1.0,     # 期間全体スケール（この値で 1日幅×bar_max_fraction×max_span_days）
        max_span_days: int = 2,         # 最大スパン（日数）
        row: int = 2,
        col: int = 1
    ):
        SIZE_BIN = 20.0
        ...
        agg["x_end"]   = agg["x_start"] + agg["length"]               # 最大 2 日先
        ...
        for _, r in agg.iterrows():
            ...
            # Hover用の不可視マーカー（shape単体はhoverできないため）
    ...
    use_correction = st.sidebar.checkbox("実測ベース補正（この版では線のみ表示）", value=False)
    tolerance_min  = st.sidebar.slider("時刻差の許容範囲（分）", 5, 120, 35, step=5)
    ...
    if period_choice == "直近2週間":
        ...
        st.sidebar.info("補正学習期間：任意期間では『選択された期間全体』で学習します。")

    with st.sidebar.expander("任意列CSV（上段右軸に重ねる）", expanded=False):
        uploaded_generic = st.file_uploader("CSVを追加（複数可）", type=["csv"], accept_multiple_files=True)

    with st.sidebar.expander("サイズCSV（2行ヘッダー：1行目=サイズ、2行目=系列）", expanded=True):
        uploaded_size_files = st.file_uploader("サイズCSVを追加（複数可）", type=["csv"], accept_multiple_files=True)
        size_series_filter  = st.text_input("seriesフィルタ（空=全て、例：w_temp）", value="")
        bar_max_fraction    = st.slider("横棒の最大長（1日比）", 0.1, 1.0, 1.0, step=0.05)
        max_span_days       = st.sidebar.slider("横棒の最大スパン（日数）", 1, 3, 2, step=1)

    with st.sidebar.expander("任意列の表示（上段右軸）", expanded=False):
    ...
    if period_choice == "直近2週間":
        ...
        if available_days:
            ...
            min_day = latest_dt_dr.date(); max_day = latest_dt_dr.date()
    ...
    show_obs_points = st.sidebar.checkbox("実測（OBS）を上段に重ねて表示", value=True)
    ...
    if uploaded_size_files:
        ...
        if size_long_list:
            ...
            default_vmax = float(
                tmp.groupby(["date_day", "size_bin"], as_index=False)["value"].sum()["value"].max()
            )
            vmax_full_day = st.sidebar.number_input(
                "横棒スケール最大値（期間全体・20刻み合算での最大長）—この値で1日幅×最大スパン",
                min_value=0.0001, value=default_vmax if default_vmax > 0 else 1.0, step=0.1
            )
    ...
    # === 上段：水温（左軸）・OBS（左軸）・任意列（右軸、opacity=0.65） ===
    ...
    selected_depths = st.multiselect("表示する水深（複数選択可）", depths_plot, default=default_depths)
    ...
    # --- 回帰補正パラメータ（元仕様）を期間に合わせて学習 ---
    reg_depthwise = None
    if use_correction:
        if period_choice == "直近2週間":
            # 表示は直近14日、学習はスライダー train_days 日ぶん（最新終端から遡り）
            train_end_dt   = pd.Timestamp(end_day) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
            train_start_dt = train_end_dt - pd.Timedelta(days=train_days)
        else:
            # 任意期間モード：選択した表示期間全体で学習
    ...
    if not df_period.empty and selected_depths:
        for d in selected_depths:
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=f"%{{x}}<br>水深: {d}m<br>水温: %{{y:.2f}} °C"
            ...
            if use_correction and (reg_depthwise is not None) and (int(d) in reg_depthwise):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate=f"%{{x}}<br>水深: {d}m<br>補正水温: %{{y:.2f}} °C<extra></extra>"
                ), row=1, col=1, secondary_y=False)
    ...
    if show_obs_points and not merged_for_points.empty and selected_depths:
        for d in selected_depths:
            ...
            if sub_obs.empty:
                continue
            obs_color = obs_color_map[int(d)]
            symbol    = symbol_map[int(d)]
            fig.add_trace(go.Scatter(
                x=sub_obs["datetime"], y=sub_obs["obs_temp"], mode="markers",
                name=f"{d}m 実測",
                marker=dict(size=6, color=obs_color, symbol=symbol, line=dict(color="black", width=0.1)),
                opacity=0.70,
                hovertemplate=f"%{{x}}<br>水深: {d}m<br>実測水温: %{{y:.2f}} °C<extra></extra>"
            ), row=1, col=1, secondary_y=False)

    # 任意列（上段・右軸、opacity=0.65）
    ...
    if uploaded_generic:
        ...
        for g in gen_list:
            g_period = g[(g["datetime"] >= pd.to_datetime(start_day)) &
                         (g["datetime"] <= pd.to_datetime(end_day) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1))].copy()
            if g_period.empty:
                continue
            value_cols = [c for c in g_period.columns if c not in ["Date","datetime","Depth","depth_m","source"]
                          and pd.api.types.is_numeric_dtype(g_period[c])]
            if not value_cols:
                continue
            g_res = g_period.sort_values("datetime") if "depth_m" in g_period.columns else \
                    g_period.sort_values("datetime").assign(depth_m=0)
            ...
            for col in cols:
                ...
                if generic_plot_type == "日別棒(平均)":
                    ...
                    if sub_line.empty:
                        continue
                    fig.add_trace(go.Scatter(
                        x=sub_line["datetime"], y=sub_line[col],
                        mode="lines", name=f"{src} {col}",
    ...
    # === X軸：実測比と同様に「自動刻み」。サイズの有無で表示段だけ切替 ===
    x_range = [pd.Timestamp(start_day), pd.Timestamp(end_day) + pd.Timedelta(days=1)]
    fig.update_xaxes(type="date", range=x_range)  # 軸型とレンジだけ明示（可変に追従）

    if has_size_data:
        # サイズ横棒あり → 下段に日付目盛・タイトルを出す／上段は隠す
    ...
    # 下段のY軸（サイズ帯）は add_size_horizontal_bars_binned20() 内で設定済み
